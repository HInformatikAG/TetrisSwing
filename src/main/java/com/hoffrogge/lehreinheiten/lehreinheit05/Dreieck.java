...
public class Dreieck implements GeometrischeFigur {
    ...
    /*
     * Definition von Variablen, sie sind als private markiert, d. h. sie koennen
     * nur in dieser Klasse genutzt werden. Werden auch Felder genannt. Variablen
     * koennen hier einen Standardwert bekommen, falls nicht, haben sie per
     * Definition einen Standardwert: 0 fuer primitive Zahlen, false fuer boolean
     * und null fuer Objekte.
     */
    /**
     ...
     * Sie gilt fuer alle Instanzen der Klasse gleich (Keyword static) und sie kann
     * nicht geandert werden (Keyword final). Sie kann in dieser Klasse und in allen
     * Kindern genutzt werden (Keyword protected).
     */
    protected static final int KANTEN_LAENGE = 100;

    /*
     * Ein Konstruktor, erkennbar daran, dass er genau so heisst, wie die Klasse. Es
     * kann mehrere Konstruktoren geben, solange sie unterschiedliche Parameter
     * haben.
     */
    public Dreieck() {
    ...
    /*
     * Methoden, die mit der Annotation @Override versehen sind, ueberschreiben die
     * Methode, die sie entweder aus einem Interface (wie GeometrischeFigur) oder
     * aus einer Elternklasse kennen (siehe Vererbung, das kommt noch). Sind sie aus
     * einem Interface, dann muessen sie ueberschrieben werden, weil ein Interface
     * die Methoden nur vorgibt, sie aber nicht selbst implementiert. Bei einer
     * vererbten Methode muss sie nicht unbedingt ueberschrieben werden.
     *
    ...
    /*
     * Diese Methode aus dem Interface GeometrischeFigur muss implementiert werden,
     * damit das GeometrischeFigur gezeichnet werden kann. Bleibt die Methode leer,
     * wird einfach nichts gezeichnet.
     */
    @Override
     ...
     * sind. Der Rest des Programs kann diese Methoden nutzen, wenn er ein Dreieck
     * als Dreieck kennt (also eine Instanz/Variable vom Typ Dreieck hat) und die
     * Methoden public sind Wenn er das Dreieck aber nur als GeometrischeFigur
     * kennt, koennen diese Methoden dort nicht genutzt werden.
     */
